...
StructuredBuffer<SFiniteVolumeBound> g_data : register( t0 );
...
int ClipAgainstPlane(const int iSrcIndex, const int iNrSrcVerts, const int subVol, const int p);
...
void main(uint threadID : SV_GroupIndex, uint3 u3GroupID : SV_GroupID)
{
	...
	const int subVol = (int) (t/8);
	const int volIndex = subVol+(int) g*8;
	const int sideIndex = (int) (t%8);
	
	SFiniteVolumeBound volDat = g_data[volIndex];
	
	const float3 vBoxX = volDat.vBoxAxisX.xyz;
	const float3 vBoxY = volDat.vBoxAxisY.xyz;
	const float3 vBoxZ = volDat.vBoxAxisZ.xyz;
	const float3 vCen = volDat.vCen.xyz;
	const float fRadius = volDat.fRadius;
	const float2 vScaleXZ = volDat.vScaleXZ;

	{
		if(sideIndex<6 && volIndex<g_iNrVisibVolumes)		// mask 2 out of 8 threads
		{
			float3 q0, q1, q2, q3;
			...
			clipFlags[subVol*6+sideIndex] = (uFlag0<<0) | (uFlag1<<6) | (uFlag2<<12) | (uFlag3<<18);

			// store in clip buffer (only use these vMin and vMax if volume is 100% visible in which case clipping isn't needed)
			posX[subVol*MAX_PNTS*2 + sideIndex] = vMin.x;
			posY[subVol*MAX_PNTS*2 + sideIndex] = vMin.y;
			posZ[subVol*MAX_PNTS*2 + sideIndex] = vMin.z;

			posX[subVol*MAX_PNTS*2 + sideIndex + 6] = vMax.x;
			posY[subVol*MAX_PNTS*2 + sideIndex + 6] = vMax.y;
			posZ[subVol*MAX_PNTS*2 + sideIndex + 6] = vMax.z;
		}
	}
	...
	{
		if(sideIndex==0 && volIndex<g_iNrVisibVolumes)
		{
			...
			for(int f=0; f<6; f++)
			{
				unsigned int uFlagAnd = clipFlags[subVol*6+f]&0x3f;
				unsigned int uFlagOr = uFlagAnd;
				for(int i=1; i<4; i++)
				{
					unsigned int uClipBits = (clipFlags[subVol*6+f]>>(i*6))&0x3f;
					uFlagAnd &= uClipBits;
					uFlagOr |= uClipBits;
				}
			...
			if(uCollectiveAnd!=0 || uCollectiveOr==0)		// all invisible or all visible (early out)
			{
				if(uCollectiveOr==0)	// all visible
				{
					for(int f=0; f<6; f++)
					{
						...
						float3 vFaceMi = float3(posX[subVol*MAX_PNTS*2 + sideIndex + 0], posY[subVol*MAX_PNTS*2 + sideIndex + 0], posZ[subVol*MAX_PNTS*2 + sideIndex + 0]);
						float3 vFaceMa = float3(posX[subVol*MAX_PNTS*2 + sideIndex + 6], posY[subVol*MAX_PNTS*2 + sideIndex + 6], posZ[subVol*MAX_PNTS*2 + sideIndex + 6]);
			...
			else		// :( need true clipping
			{
				...
				for(int f=0; f<6; f++)
				{
					...
					int offs = iSrcIndex*MAX_PNTS+subVol*MAX_PNTS*2;
					...
					for(int p=0; p<6; p++)
					{
						const int nrVertsDst = ClipAgainstPlane(iSrcIndex, iNrSrcVerts, subVol, p);
					...
					if(iNrSrcVerts>2)
					{
						int offs_src = iSrcIndex*MAX_PNTS+subVol*MAX_PNTS*2;
				...
				////////////////////// look for camera frustum verts that need to be included. That is frustum vertices inside the convex hull for the volume
				...
				for(int i=0; i<8; i++)	// establish 8 camera frustum vertices
				{
					...
					posX[subVol*MAX_PNTS*2 + i] = vViewSpace.x;
					posY[subVol*MAX_PNTS*2 + i] = vViewSpace.y;
					posZ[subVol*MAX_PNTS*2 + i] = vViewSpace.z;
				}
				...
				for(int f=0; f<6; f++)
				{
					...
					if(fLen>1) vN = normalize(vN);		// this won't necessarily be a non zero vector (spot volumes have all 4 top points as the same)

					for(int i=0; i<8; i++)
					{
						float3 vViewSpace = float3(posX[subVol*MAX_PNTS*2 + i], posY[subVol*MAX_PNTS*2 + i], posZ[subVol*MAX_PNTS*2 + i]);
			...
			// to see if the volume is occluded: vMin.z*VIEWPORT_SCALE_Z > MipTexelMaxDepth
			g_vBoundsBuffer[volIndex+0] = float3(0.5*vMin.x+0.5, -0.5*vMax.y+0.5, vMin.z*VIEWPORT_SCALE_Z);
			g_vBoundsBuffer[volIndex+g_iNrVisibVolumes] = float3(0.5*vMax.x+0.5, -0.5*vMin.y+0.5, vMax.z*VIEWPORT_SCALE_Z);
		}
	}
}
...
{
	int offs_src = iSrcIndex*MAX_PNTS+subVol*MAX_PNTS*2;
	int offs_dst = (1-iSrcIndex)*MAX_PNTS+subVol*MAX_PNTS*2;
